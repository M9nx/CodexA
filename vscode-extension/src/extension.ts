...
// ══════════════════════════════════════════════════════════════════════
// PANEL 4: TOOLS (AI agent tool runner + doctor)
// ══════════════════════════════════════════════════════════════════════
class ToolsViewProvider implements vscode.WebviewViewProvider {
    ...
    wv.webview.onDidReceiveMessage(async (msg) => {
      if (msg.type === "doctor") {
        ...
        try {
          ...
          const args = ["tool", "run", msg.tool, "--json"];
          if (msg.args) {
  ...
  private _html(): string {
    return /* html */ `<!DOCTYPE html><html><head><meta charset="UTF-8">
<style>${SHARED_CSS}
  .btn-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 4px; margin-bottom: 8px; }
  ...
  <div class="tool-form">
    ...
    <textarea id="tool-args" rows="3" placeholder="symbol_name=MyClass&#10;file_path=src/main.py"></textarea>
    <button id="run-tool-btn" style="width:100%; margin-top:4px">Run Tool</button>
  </div>
  <div id="status" class="status"></div>
  ...
  <script>${SHARED_JS}
    ...
    document.getElementById('doctor-btn').addEventListener('click', () => {
      post('doctor', {}); statusEl.textContent = 'Running doctor…'; outputEl.innerHTML = '';
    });
    document.getElementById('index-btn').addEventListener('click', () => {
      post('index', {}); statusEl.textContent = 'Indexing…'; outputEl.innerHTML = '';
    });
    document.getElementById('models-btn').addEventListener('click', () => {
      post('models', {}); statusEl.textContent = 'Loading…'; outputEl.innerHTML = '';
    });
    document.getElementById('tools-btn').addEventListener('click', () => {
      post('tool-list', {}); statusEl.textContent = 'Loading…'; outputEl.innerHTML = '';
    });
    ...
    window.addEventListener('message', e => {
      ...
      if (msg.type === 'doctor-result' || msg.type === 'index-result') {
        outputEl.innerHTML = '<pre>' + escapeHtml(msg.data) + '</pre>';
      } else if (msg.type === 'models-result') {
        const models = Array.isArray(msg.data) ? msg.data : [];
        outputEl.innerHTML = models.map(m =>
          '<div class="card"><div class="card-title">' + escapeHtml(m.display_name || m.name)
        ...
        outputEl.innerHTML = tools.map(t =>
          '<div class="card"><div class="card-title">' + escapeHtml(t.name) + '</div>'
          + '<div class="card-meta">' + escapeHtml(t.description || '') + '</div></div>'
        ).join('') || '<div class="empty">No tools.</div>';
      } else if (msg.type === 'tool-run-result') {
        outputEl.innerHTML = '<pre>' + escapeHtml(JSON.stringify(msg.data, null, 2)) + '</pre>';
      } else if (msg.type === 'error') {
        outputEl.innerHTML = '<div class="empty" style="color:var(--vscode-errorForeground)">' + escapeHtml(msg.message) + '</div>';
      }
    });
